...
public class GameManager : MonoBehaviour {
    ...
    private byte[] SerializeToJson(APIForm data)
    {
        string json = JsonConvert.SerializeObject(data);
        return Encoding.UTF8.GetBytes(json);
    }

    public byte[] SimulateSpin(string token, int totalBet, int betLine, int betValue)
    {
        if (string.IsNullOrEmpty(token) || totalBet <= 0 || betLine <= 0 || betValue <= 0)
        {
            return SerializeToJson(new APIForm
            {
                status = false,
        ...
        for (int i = 0; i < 15; i++)
        {
            randomNums[i] = UnityEngine.Random.Range(1, 11);
        }

        // Generate unique winpaylines based on betLine (Ensuring max length is 25)
        int winpaylinesCount = Math.Min(1, betLine); 
        HashSet<int> winpaylinesSet = new HashSet<int>();

        while (winpaylinesSet.Count < winpaylinesCount)
        {
            winpaylinesSet.Add(UnityEngine.Random.Range(0, 15));
        }
        int[] winpaylines = new List<int>(winpaylinesSet).ToArray();

        // Calculate money result
        int moneyResult = winpaylines.Length * betValue;
